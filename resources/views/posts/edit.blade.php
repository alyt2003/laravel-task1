...
@section('page-subtitle', 'Update this post\'s content or owner.')
            ...
            <form method="POST" action="{{ route('admin.posts.update', $post) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="content" name="content" required rows="6"
                        class="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('content', $post->content) }}</textarea>
                </div>
